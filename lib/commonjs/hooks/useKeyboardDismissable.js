...
function useBackHandler({
  ...
  (0, _react.useEffect)(() => {
    ...
    // with a `.remove()` method. Older RN exposes removeEventListener on BackHandler.
    let subscription;
    ...
    if (enabled) {
      try {
        ...
        // In case the environment throws, leave subscription undefined and rely on fallback
    ...
    return () => {
      if (subscription && typeof subscription.remove === 'function') {
        subscription.remove();
      } else if (_reactNative.BackHandler.removeEventListener) {
